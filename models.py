...
class FilesystemObject(object):
    ...
    def __init__(self, filename, post=None, root=None):
        ...
        self.date = datetime.datetime.fromtimestamp(os.path.getmtime(os.path.join(self.root_dir,filename))).strftime("%d-%m-%Y")
    ...
    def all(cls, root):
        ...
        """
        arr = os.listdir(root)
        list_of_files = sorted( arr, key = lambda x: os.path.getmtime(os.path.join(root, x)), reverse=True)
        return [cls(filename=x, root=root) for x in list_of_files]
