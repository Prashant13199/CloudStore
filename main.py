...
ALLOWED_EXTENSIONS_DOCUMENT = ['.pdf', '.html', '.xml' , '.py', '.pages', '.numbers', '.keynote', '.xlsx', '.csv', '.docx', '.pptx', '.css', '.js', '.txt']
...
def switch():
    try:
        ...
        host = ', '.join(hosts[1])
        return redirect('http://{}:3000'.format(host))
    except:
    ...
    if request.method == 'POST':
        ...
        if user:
            if password in user[1]:
                ...
                session['DOWNLOAD_ROOT_DIR'] = 'data/{}/Files/Downloads/'.format(session['userid'])
                try:
...
def register():
    if request.method == 'GET':
        ...
        user = 'admin'
        sql = "INSERT INTO users (email, password, name, user) VALUES ('{}', '{}', '{}', '{}')".format(email,password, name, user)
        mycursor.execute(sql)
        mydb.commit()
        userid = email
...
def edit_user():
    ...
    user = request.form['user']
    email = request.form['email']
    print(name,password,user,email)
    sql = "UPDATE users set name = ('{}'), password = ('{}'), user = ('{}') WHERE email = ('{}')".format(name,password,user,email)
    mycursor.execute(sql)
...
def logout():
    try:
        os.remove(os.path.join(os.path.join(session['FILE_ROOT_DIR'],"Files.zip")))
    except:
        pass
    try:
        os.remove(os.path.join(os.path.join(session['FILE_ROOT_DIR'],"shared.zip")))
    except:
        pass
    session.clear()
    return render_template("signin.html")
...
def show_gallery():
    try:
        if session.get("name"):
            ...
            return render_template('image.html', images=arr,year = year)
        else:
...
def sharefiles(emailto):
    try:
        ...
        for i in data:
            ...
            path = '/'.join(path[:-1])
            id = filename+session['email']
            sql = "INSERT INTO shared (filename, path, email, emailto,date, id) VALUES ('{}', '{}' , '{}', '{}', '{}', '{}')".format(filename,path, session['email'],emailto,date1,id)
            mycursor.execute(sql)
            mydb.commit()
    ...
    try:
        if session.get("email") in subpath:
            ...
            if ext not in ALLOWED_EXTENSIONS_AUDIO and ext not in ALLOWED_EXTENSIONS_VIDEO and ext not in ALLOWED_EXTENSIONS_DOCUMENT and ext not in ALLOWED_EXTENSIONS_IMAGE:
                ...
                path1 = subpath.split('/')
                path1 = path1[2:]
                return render_template('file.html', files=files, path = subpath, path1=path1, lst=lst)
            ...
            if ext == '.zip':
                ...
                path1 = subpath.split('/')
                path1 = path1[2:]
                multiprocessing.Process(target=unzip,args=(path,subpath)).start()
    ...
    for k in videos:
        for j in k:
            ...
            if ext in ALLOWED_EXTENSIONS_VIDEO:
                arr.append(j)
    video = random.choice(arr)
    return render_template('playvideo.html',path=subpath,name=filename,video=video,n=nam)
...
def showfile1(path, name):
    try:
        if session.get("email") in path:
            ...
            path2 = path1.split('/')
            path2 = path2[2:]
            return render_template('file.html', files=files,path=path1,path1=path2,lst=lst)
    except:
        return redirect(url_for('error'))
...
def downloadfiles():
    try:
        ...
        zipfile(names,path)
        return jsonify({'message':'Files.zip created','urll': url_for('downfile')})
    except:
        ...
        if session.get("name"):
            ...
            if not ext:
                ...
                with ZipFile('shared.zip','w') as zip:
                    for file in file_paths:
                        zip.write(file)
                source = os.path.join(os.getcwd(),'shared.zip')
                destination = os.path.join(session['FILE_ROOT_DIR'],'shared.zip')
                shutil.move(source, destination)
                return send_from_directory(session['FILE_ROOT_DIR'], "shared.zip", as_attachment=True)
            name, ext = os.path.splitext(filename)
            ext = ext.lower()
            if ext in ALLOWED_EXTENSIONS_VIDEO:
    ...
    try:
        if session.get("name"):
            ...
            for i in file1:
                ...
                current_time1 = datetime.datetime.strptime(current_time, "%d-%m-%Y")
...
def emptytrash():
    try:
        ...
        return jsonify({"message":"Trash empty"})
    except:
        return jsonify({"message":"Not able to Delete"})
...
def download_file():
    ...
    multiprocessing.Process(target=downloadfile,args=(file_url,path,)).start()
    flash("File")
    return redirect(url_for('download'))
...
def download_torrent():
    ...
    path = os.path.join('/home/pi/mystorage',session['DOWNLOAD_ROOT_DIR'])
    multiprocessing.Process(target=downloadtorrent,args=(magnet_link,path,)).start()
    flash("Torrent")
    return redirect(url_for('download'))
...
def profile():
    if request.method=="GET":
        sql = "SELECT * from users WHERE email = ('{}')".format(session['email'])
        mycursor.execute(sql)
        user = mycursor.fetchone()
        ...
        name = request.form['name']
        password = request.form['password']
        sql = "UPDATE users set name = ('{}'), password = ('{}') WHERE email = ('{}')".format(name,password,session['email'])
        mycursor.execute(sql)
        flash("Details Updated")
...
def deleteimage():
    path = os.path.join(os.getcwd(),'data/{}/profile.png'.format(session['email']))
    try:
        os.remove(path)
        session['pic'] = " "
